...
def _flush_vad(vad: Any) -> None:
    ...
    Пытается вызвать ``vad.flush()`` если он есть. В sherpa-onnx некоторые
    версии принимают финальный chunk через ``accept_waveform(is_final=True)``,
    другие имеют метод ``flush()`` или ``finalize()``. Изолировано здесь
    чтобы при несовпадении API адаптация была в одном месте.

    TODO(python-dev): свериться с актуальной sherpa-onnx 1.12+ docs;
    если API другой — адаптировать.
    """
    flush = getattr(vad, "flush", None)
    if callable(flush):
        flush()
        return
    # Fallback: некоторые версии требуют просто дождаться через пустой
    # accept_waveform. Оставляем no-op: последние сегменты могут быть
    # отброшены. См. TODO выше.
