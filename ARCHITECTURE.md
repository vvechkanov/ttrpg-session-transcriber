...
Этот документ описывает **архитектуру кода**: шесть слоёв, направление зависимостей,
публичные контракты и зафиксированные решения (ADR).

> **Статус документа (2026-08-03).** Написан весной 2026 перед стартом «Приоритета 2»
> как описание *целевого* состояния. Целевое состояние **достигнуто**: шесть слоёв
> внедрены, контракты из §5 живут в коде. Поэтому разделы §3–§10 читаются как описание
> настоящего, а не будущего — с тремя оговорками:
>
> - **UI-слой описан устаревше.** Везде, где сказано «tkinter widgets» и `ui/gui.py`,
>   в реальности PySide6/QML: `ui/models` (QAbstractListModel + Q_PROPERTY),
>   `ui/engines` (QThread-воркеры), `ui/qml` (экраны). См. ADR-017.
> - **Pipeline flow неполон.** Стадий семь: `start → speech → chat → combat → merge →
>   render → chunk → done`; стадии `combat` и `chunk` появились позже.
> - **Формулировки «в Приоритете N сделаем»** — исторические. Актуальный план работ —
>   в [TASKS.md](TASKS.md), продуктовое «зачем» — в [VISION.md](VISION.md).
>
> Полная актуализация — задача §3.5 в TASKS.md. Что уже описано верно: границы слоёв,
> запреты импортов, контракты `Annotation` / `Timeline` / `ScriptEvent` / `Source` /
> `Merger` / `Renderer`, ADR.
...
**В документе есть:**
- Слои (6 папок), направление зависимостей, ответственности
...
- План работ и приоритеты — в `TASKS.md`, продуктовое видение — в `VISION.md`
...
## 2. Откуда пришли (исторический раздел)

До весны 2026 проект был слабо структурированным монолитом:

```
scripts/wisper_launcher.py    (1003 строки — argparse CLI + tkinter GUI +
                               subprocess whisperx + оркестрация + GPU-чек)
scripts/merge_whisperx.py     (168 строк — engine-agnostic by design)
scripts/parse_fvtt_chat.py    (изолирован, чистые функции)
```

Всё три файла **удалены**; их логика живёт в слоях: оркестрация — в `core/pipeline.py`,
мерж — в `mergers/script_merger.py`, парсер чата — в `sources/game_log/fvtt_chat.py`,
CLI — в `ui/cli.py`, GUI — в `ui/qml` + `ui/models`.

Чем монолит был плох (и что именно чинили слои):

- пять ответственностей в одном файле — правка GUI ломала CLI;
- транскрипция прибита к `whisperx` через subprocess — альтернативный движок требовал
  `if/else` в оркестраторе;
- мерджер умел только склеивать соседние реплики одного говорящего по gap — вставить
  чат или боевой лог равноправным элементом было некуда;
- тестов не было.

Сейчас: два ASR-бэкенда в реестре (плюс legacy-whisperx), чат и боевой лог как
полноправные источники, ~490 тестов и CI-матрица.
...
## 7. Migration plan (исторический раздел — миграция выполнена)

> Пункты 1–10 ниже сделаны весной 2026, «Приоритеты 3–6» тоже пройдены (тесты и CI есть,
> GigaAM-бэкенд есть, UI мигрировал сразу на PySide6/QML минуя tkinter-вариант — ADR-017).
> Раздел оставлен как запись о том, что и почему делалось разом.
