...
В Python нет enforced module boundaries (в отличие от gradle modules в Kotlin), поэтому правила зависимостей и запрет на импорт между независимыми соседями проверяет `import-linter`: контракты лежат в `pyproject.toml` (`[tool.importlinter]`), гейт — job `layers` в `.github/workflows/ci.yml`. Решение записано в ADR-023 (`docs/adr/`), оно же отменяет строку последствий ADR-10 «dependency rules проверяются ревьюером + ruff, не build system».

Машиной покрыт не весь список выше. Строка «`core` знает про tkinter / PySide6 или любой GUI widget API» — про сторонние пакеты, а не про рёбра между слоями, и ни один из двух контрактов её не проверяет; отдельный контракт для неё зелёный сегодня, но заведён карточкой, а не включён (ADR-023, «Рассмотрено и отвергнуто»). «Любой циклический импорт между слоями» отдельным контрактом не выражен — сегодня его закрывает сам контракт `layers`, потому что цикл между слоями обязан содержать ребро вверх.
...
> Причина в §4.1: GUI не зовёт `core.pipeline.run`, а
> воспроизводит его шаги у себя, и вместе с шагами утащил зависимости.
> Починка запланирована отдельно как F-C1 (`TASKS.md`, задача 1.3).
>
> **Список больше не «составлен чтением и может быть неполон» — его
> считает машина.** Нарушающих рёбер ровно `8`, и все они перечислены
> поимённо в `ignore_imports` контракта «ui ходит в нижние слои только
> через core»; девятое такое ребро роняет CI. Перечислены именно рёбра, а не каталог
> целиком: все восемь живут в `ui/engines`, и оптовое послабление увело бы
> из-под гейта тот самый модуль, где нарушения и заводятся.
>
> Важно, чем это НЕ проверяется: контракт типа `layers` здесь бесполезен.
> Он запрещает импорт вверх и между независимыми соседями, а импорт вниз
> через уровень разрешает — а все восемь именно такие, и на них он
> показывает `KEPT`. Поэтому смежность `ui` → `core` стоит отдельным
> контрактом `forbidden` с `allow_indirect_imports`.
>
> Цикл `ui/models` ↔ `ui/engines` (§4.1) гейт не ловит и ловить не должен:
> §3 запрещает цикл **между слоями**, а этот — внутри одного.
...
- (+) Dependency rules проверяются ревьюером + ruff, не build system. — **Отменено ADR-023.** Строка стояла с самого первого коммита репозитория и была неверна в обеих половинах: ruff правил импорта никогда не проверял (в конфиге только `E4,E7,E9,F`), а ревьюер не увидел восьми нарушений §3, из которых два — ленивые импорты внутри методов. Само решение ADR-10 (папка = слой, без `src/`-layout) в силе: `import-linter` читает исходники и папочной раскладке не мешает.
