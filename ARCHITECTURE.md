...
В Python нет enforced module boundaries (в отличие от gradle modules в Kotlin), поэтому правила выше проверяет `import-linter`: контракты лежат в `pyproject.toml` (`[tool.importlinter]`), гейт — job `layers` в `.github/workflows/ci.yml`. Решение записано в ADR-023 (`docs/adr/`), оно же отменяет строку последствий ADR-10 «dependency rules проверяются ревьюером + ruff, не build system».
...
> считает машина, и их ровно восемь.** Все восемь перечислены поимённо
> в `ignore_imports` контракта «ui ходит в нижние слои только через core»,
> девятое такое ребро роняет CI. Перечислены именно рёбра, а не каталог
