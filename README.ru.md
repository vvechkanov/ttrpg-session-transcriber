...
GigaAM-v3 — открытая русская ASR-модель Сбера под MIT. На русском аудио стабильно обгоняет Whisper. Также поддерживает **контекстный байасинг** — можно дать ей список имён NPC, и она будет узнавать их корректно вместо того чтобы изобретать креативные варианты. Список hotwords содержит имена PF2e — но байасинг в этой сборке выключен, см. ниже.
...
Сам пайплайн транскрипции универсален. Мы тестируем на Pathfinder 2e потому что мейнтейнер играет в неё. Список hotwords для PF2e лежит в `sources/speech/_gigaam_download.py` и пишется в бандл GigaAM как `hotwords.txt` при установке. Контекстный байасинг в этой сборке выключен (`ENABLE_HOTWORDS_BIASING`), поэтому правка списка на вывод пока не влияет, и пользовательского способа подставить свой нет.
