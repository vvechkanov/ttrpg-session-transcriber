# Figma Make Prompt — Settings Drawer · Аудио (Whisper / faster-whisper)

**Цель:** второй вариант контента боковой панели настроек — для модуля
"Аудио · Whisper". Используется тем же темплейтом `audio_source_template`,
что и GigaAM, но с другими `params`. Этот мокап доказывает, что один
темплейт обслуживает несколько backend'ов.

**Как использовать:** открой в Figma Make тот же проект, что делал
Screen 3 Session Detail и GigaAM settings drawer. НЕ начинай новый чат.
Вставь всё между `---BEGIN---` и `---END---`.
...
- The point of this second prompt is the A/B comparison with the GigaAM
  panel — when you place them side by side, the identical parts (header,
  files, speakers, footer) should visually confirm that they share a
  template, and the engine section should visually confirm that params
  drive the differences.
- If Figma Make creates a wildly different layout for the Whisper
  version, push back with "Keep the structure identical to the GigaAM
  settings panel. Only the engine section and section 5 differ".
- If the advanced section starts feeling overwhelming, push back with
  "Hide no_speech_threshold and temperature behind a secondary «совсем
  продвинутые» sub-accordion inside the Advanced section".
