...
| Кол-во ASR-бэкендов | Роста не планируем → единый реестр (F-C3) и дедуп install (F-C5) демотированы (B2). Писалось про три; с удалением WhisperX (C2) их два, и решение от этого только крепче |
...
рекомендует удалить вместе с WhisperX. **Посылка «скрипт ставит именно его»
измерена и оказалась неверной** (16.09, см. задачу 1.8): скрипт ставит venv,
torch и ffmpeg, а whisperx — последние ~20 строк из 186. Задача 1.8 поэтому
не вошла в PR по 1.5 и ждёт ответа на вопрос «чем заменяется установка под
Windows».
...
- [x] Альтернатива/дополнение: `import-linter` с контрактами из ARCHITECTURE.md §3 — сделано 20.09, два контракта в `pyproject.toml`, решение в ADR-023
- [x] Job в ci.yml — **блокирующий** (нарушение границ слоёв валит сборку) — сделано 20.09, отдельный job `layers`, а не `lint`: красный контракт слоёв и красный F821 — разные отказы
