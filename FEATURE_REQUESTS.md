...
>
> *Закрыто итерацией 3b: `TimelineWindow.extended_for_track` двигает правый
> край под длительность дорожки, `SessionMeta._grow_window_for_track` зовёт
> его из `setTotalSeconds`, а `SourceListModel` пересчитывает проценты строк
> по `timelineWindowChanged`. Замер оказался мягче правды: дело не только в
> сдвиге полос на несколько пунктов — когда запись длиннее чата, её хвост с
> оси срезается совсем, а осциллограмма растягивается на окно короче звука,
> так что каждый пик стоит не на своём настенном времени.*

> **Уточнение владельца (21.04, на момент заявки):** UI уже концептуально
> устроен как таблица: слева гаттер 220 px (имена), справа общая ось
> времени. Все элементы (чат, бой, Craig-сегменты, треки) — ряды на этой оси
> с позициями `startPct..endPct`. Проблема не в слоях, а в том, что оффсеты
> тогда были fake.
...
**Статус:** ✅ ось абсолютная, бои парсятся, окно растёт под длину записи
...
`chat_range` из `chat_paths[0]`, и это сознательное решение, записанное
там же в комментарии. Мерж берёт первый **того же** набора: и ось, и
`core/pipeline.py`, и баннер покрытия зовут теперь один искатель
`core/file_matchers.py::detect_fvtt_chat_logs` и берут его `[0]`. Значит
сообщения из второго лога не попадут ни в окно, ни в `merged.txt`, и
счётчика или предупреждения об этом нет — но расходиться в том, *какой*
файл считается первым, стороны больше не могут.
Заведено: https://trello.com/c/xS6eXH8L
...
**Итерация 3b ✅:** дефект окна из блока выше закрыт. `build_window`
по-прежнему зовётся один раз с `max_track_duration=None` — длительности к
этому моменту не готовы, — но окно больше не остаётся таким:
`TimelineWindow.extended_for_track` двигает правый край, как только
`durationReady` приносит настоящую длину, а `SourceListModel`
пересчитывает проценты полос по `timelineWindowChanged`. Растёт только
`t_end`: `t0` — нулевая точка, от которой `ui/engines/asr_worker` меряет
офсеты сегментов речи, и её сдвиг менял бы `merged.txt`.
...
Известное ограничение: исхода боя в подвале нет. `_summarise`
(`renderers/combat_aware.py:287`) берёт из дампа только раунды и XP —
это записано в его докстринге как решение, а не недосмотр, — и ни
источник, ни рендерер слова «победа» не производят. Пример выше
исправлен на то, что печатается на самом деле.
