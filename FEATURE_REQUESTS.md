...
Оффсеты больше не fake: `TrackListModel.loadFromDir` зовёт `build_window`,
и `startPct`/`endPct` считаются по фактическому времени. Абзац выше оставлен
как запись исходной заявки — он описывает 21.04, а не сегодняшний код.
