...
**Как читать файл.** Каждая фича растёт сверху вниз: сначала баннеры закрытых итераций
(свежие, с датами и коммитами), ниже — исходный запрос и **статус на момент заведения**.
Нижний блок исторический: если он противоречит баннеру — прав баннер.

**Куда это встраивается:** порядок работ и ступени — в [TASKS.md](TASKS.md), продуктовое
«зачем» — в [VISION.md](VISION.md).
...
| # | Фича | Статус | Куда едет |
|---|---|---|---|
| #1 | Инсталлер | 🅿️ запаркован | Ступень 5 (как продукт); сборка в CI — Ступень 1 |
| #2 | Глобальные настройки ASR | ✅ готово | — |
| #3 | Единая ось времени | ⚠️ 3a готова, хвост 3b | Ступень 0, хвост (`TASKS.md` §3.1–3.2) |
| #4 | Несколько Craig-архивов | ✅ готово (4a + 4b) | — |
| #5 | Редактор speaker_map | ✅ готово (5b) | — |
| #6 | LLM-мержер | 📋 спроектирован | Ступень 4 |
| #7 | Настройки чанкера | ✅ готово | — |
| #8 | Combat-aware renderer | 🔮 разблокирован | Ступень 5 |
| #9 | Per-track ASR overrides | 🔮 контракт готов | Инженерный фундамент (`TASKS.md` §4.7) |
...
**Что осталось (итерация 3b):** пересчёт окна по реальной длительности дорожек и
абсолютные часы на линейке — `TASKS.md` §3.1 и §3.2. Пункты 1–4 из списка «Что делать»
ниже **закрыты**: `CombatDumpSource` (`sources/game_log/combat_dump.py`, коммит
`053a005`) парсит боевые выгрузки, стадия `combat` наполняет `Timeline.game_log`,
`SourceListModel` считает реальные проценты через `TimelineWindow`.

**Статус на момент заведения фичи (исторический):** ⚠️ каркас готов, парсинг таймстемпов нет
...
**Статус на момент заведения фичи (исторический — закрыт итерациями 4a/4b выше):** ❌ не поддерживается
...
> **Обновление 2026-08-03:** #4 закрыт (4a + 4b, 2026-04-21), контракт canonical JSON
> стабилизирован, блокер снят. Фича целиком равна Ступени 4 ТЗ — см. `TASKS.md` §6.
...
**Статус на момент заведения фичи (исторический — закрыт коммитами 7A/7B/7C выше):** ⚠️ core есть, пайплайн не зовёт, UI нет
...
**Статус:** ❌ не реализовано, но **разблокировано** (2026-08-03)
- `RENDERERS` реестр есть ([core/pipeline.py](core/pipeline.py)),
  но только `plain-text`.
- ~~**Блокер:** требует #3 (combat-aware Timeline с `game_log`)~~ — снят:
  `CombatDumpSource` наполняет `Timeline.game_log` типизированными событиями
  (коммит `053a005`). Реализация едет на Ступень 5, см. `TASKS.md` §6.
