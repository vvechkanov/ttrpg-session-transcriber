...
APPROVAL_SIGN_REGIONS = {
    "which row of the summary is the sign": (
        ...
        (
            ...
            phrase("`get_reviews` отдаёт **всю историю ревью PR**"),
            phrase(
                "«Чисто» означает: среди всех страниц нет ни одного объекта "
                "ревью Codex, чей `commit_id` равен текущему HEAD."
            ),
        ),
    ),
    ...
    "what to do when no sign is there": (
        ...
        (
            ...
            phrase("через «начинается с», а не на равенство"),
            phrase(
                "**Ячейки таблицы читаются вхождением, а не равенством, и "
                "это не только про SHA.**"
            ),
            phrase(
