...
работающий pytest портит его глобальное состояние. Рекурсии при этом не
было бы — подпроцесс импортирует модуль, но тестовую функцию не
исполняет, — и сказано это здесь потому, что «а вдруг рекурсия»
выглядит достаточным доводом и избавляет от поиска настоящего.
...
def _collected_files() -> set[str]:
    ...
    try:
        proc = subprocess.run(
            ...
            # тогда не видно вовсе. Проверено — до таймаута сбор с
            # `sleep(15)` в импорте честно ждал все пятнадцать секунд.
            timeout=300,
        )
    except subprocess.TimeoutExpired as exc:
        # Перехват не косметика: без него читатель получает трейсбек, а
        # вывод подпроцесса (в котором и видно, на каком модуле сбор
        # встал) не показывается вовсе.
        stdout = (exc.stdout or b"").decode(errors="replace") if isinstance(exc.stdout, bytes) else (exc.stdout or "")
        stderr = (exc.stderr or b"").decode(errors="replace") if isinstance(exc.stderr, bytes) else (exc.stderr or "")
        pytest.fail(
            f"pytest --collect-only не уложился в {exc.timeout} с — сбор "
            "где-то висит. Последний вывод подпроцесса:\n"
            f"{stdout[-4000:]}\n{stderr[-4000:]}"
        )
